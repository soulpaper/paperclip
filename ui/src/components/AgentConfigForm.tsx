      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? t("common.working") : t("common.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentConfig.nameLabel")} hint={help.name}>
              <DraftInput
            ...
            <Field label={t("agentConfig.titleLabel")} hint={help.title}>
              <DraftInput
            ...
            <Field label={t("agentConfig.capabilitiesLabel")} hint={help.capabilities}>
              <MarkdownEditor
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("agentConfig.promptTemplateHint")}
                </div>
              </>
            )}
          </div>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                {t("agentConfig.promptTemplateHintCreate")}
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agentConfig.bootstrapPromptLegacyHint")}
                  </div>
                </>
              )}
              {adapterType === "claude_local" && (
              ...
              <Field label={t("agentConfig.envVars")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useTranslation();
  const statusLabel =
    result.status === "pass" ? t("agentConfig.statusPassed") : result.status === "warn" ? t("agentConfig.statusWarnings") : t("agentConfig.statusFailed");
  const statusClass =
    result.status === "pass"
  ...
  return (
    <div className="space-y-1.5">
      {rows.map((row, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-1.5">
            ...
            >
              <option value="plain">{t("agentConfig.envTypePlain")}</option>
              <option value="secret">{t("agentConfig.envTypeSecret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                <button
                  ...
                  title={t("agentConfig.createSecretFromPlain")}
                >
                  {t("agentConfig.newSecret")}
                </button>
              </>
            ) : (
              <>
                <input
                  ...
                  placeholder={t("agentConfig.valuePlaceholder")}
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("agentConfig.paperclipVarsHint")}
      </p>
    </div>
  );
}
...
function ModelDropdown({
  ...
  return (
    <Field label={t("agentConfig.modelLabel")} hint={help.model}>
      <Popover
        open={open}
        ...
        <PopoverTrigger asChild>
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? t("agentConfig.defaultModel") : required ? t("agentConfig.selectModelRequired") : t("agentConfig.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          ...
          {onDetectModel && !detectedModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? t("agentConfig.detecting") : (detectModelLabel ?? t("agentConfig.detectFromConfig"))}
            </button>
          )}
...
function ThinkingEffortDropdown({
  ...
  function translateOptionLabel(id: string): string {
    const keyMap: Record<string, string> = {
      "": "agentConfig.effortAuto",
      minimal: "agentConfig.effortMinimal",
      low: "agentConfig.effortLow",
      medium: "agentConfig.effortMedium",
      high: "agentConfig.effortHigh",
      max: "agentConfig.effortMax",
      plan: "agentConfig.autocompletePlan",
      ask: "agentConfig.autocompleteAsk",
    };
  ...
  return (
    <Field label={t("agentConfig.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected ? translateOptionLabel(selected.id) : t("agentConfig.effortAuto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          {options.map((option) => (
            ...
            >
              <span>{translateOptionLabel(option.id)}</span>
