...
const quickFilterPresets = [
  { key: "all", labelKey: "issuesList.allFilter", statuses: [] as string[] },
  { key: "active", labelKey: "issuesList.activeFilter", statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { key: "backlog", labelKey: "issuesList.backlogFilter", statuses: ["backlog"] },
  { key: "done", labelKey: "issuesList.doneFilter", statuses: ["done", "cancelled"] },
];
  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        aria-label={t('issuesList.searchAriaLabel')}
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t('issuesList.filtersCount', { count: activeFilterCount }) : t('common.filter')}</span>
                {activeFilterCount > 0 && (
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-muted-foreground">{t('issuesList.statusFilter')}</span>
                  ...
                  <div className="space-y-3">
                    {/* Priority */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t('issuesList.priorityFilter')}</span>
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                    ...
                    {labels && labels.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t('issuesList.labelsFilter')}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {labels.map((label) => (
                    ...
                    {projects && projects.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t('issuesList.projectFilter')}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t('issuesList.noMatchingIssues')}
