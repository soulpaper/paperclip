...
export function OnboardingWizard() {
  ...
  function reset() {
    ...
    setTaskTitle(t("onboarding.defaultTaskTitle"));
    setTaskDescription(t("onboarding.defaultTaskDescription"));
  ...
  async function runAdapterEnvironmentTest(
    ...
    if (!createdCompanyId) {
      setAdapterEnvError(t("onboarding.selectCompanyBeforeTest"));
      return null;
    }
    ...
    try {
      ...
      setAdapterEnvError(
        err instanceof Error ? err.message : t("onboarding.envTestFailed")
      );
      return null;
    } finally {
      setAdapterEnvLoading(false);
  ...
  async function handleStep2Next() {
    ...
    try {
      if (adapterType === "opencode_local") {
        ...
        if (!selectedModelId) {
          setError(t("onboarding.opencodeRequiresModel"));
          return;
        }
        ...
        if (adapterModelsLoading || adapterModelsFetching) {
          setError(t("onboarding.opencodeModelsLoading"));
          return;
        }
  ...
  async function handleUnsetAnthropicApiKey() {
    ...
    try {
      ...
      if (result?.status === "fail") {
        setError(t("onboarding.retriedWithUnsetKey"));
      }
    } catch (err) {
      setError(
          ...
          >
            <div className="w-full max-w-md mx-auto my-auto px-8 py-12 shrink-0">
              ...
              <div className="flex items-center gap-0 mb-8 border-b border-border">
                {(
                  [
                    { step: 1 as Step, labelKey: "onboarding.stepCompany", icon: Building2 },
                    { step: 2 as Step, labelKey: "onboarding.stepAgent", icon: Bot },
                    { step: 3 as Step, labelKey: "onboarding.stepTask", icon: ListTodo },
                    { step: 4 as Step, labelKey: "onboarding.stepLaunch", icon: Rocket }
                  ] as const
                      ...
                      {[
                        {
                          ...
                          labelKey: "onboarding.claudeCode",
                          icon: Sparkles,
                          descKey: "onboarding.claudeDesc",
                          recommended: true
                        },
                        {
                          value: "codex_local" as const,
                          labelKey: "onboarding.codex",
                          icon: Code,
                          descKey: "onboarding.codexDesc",
                          recommended: true
                        }
                      ].map((opt) => (
                        <button
                        ...
                        {[
                          {
                            ...
                            labelKey: "onboarding.geminiCli",
                            icon: Gem,
                            descKey: "onboarding.geminiDesc"
                          },
                          {
                            value: "opencode_local" as const,
                            labelKey: "onboarding.openCode",
                            icon: OpenCodeLogoIcon,
                            descKey: "onboarding.openCodeDesc"
                          },
                          {
                            value: "pi_local" as const,
                            labelKey: "onboarding.pi",
                            icon: Terminal,
                            descKey: "onboarding.piDesc"
                          },
                          {
                            value: "cursor" as const,
                            labelKey: "onboarding.cursor",
                            icon: MousePointer2,
                            descKey: "onboarding.cursorDesc"
                          },
                          {
                            value: "hermes_local" as const,
                            labelKey: "onboarding.hermesAgent",
                            icon: HermesIcon,
                            descKey: "onboarding.hermesDesc"
                          },
                          {
                            value: "openclaw_gateway" as const,
                            labelKey: "onboarding.openClawGateway",
                            icon: Bot,
                            descKey: "onboarding.openClawDesc",
                            comingSoon: true,
                            disabledLabelKey: "agentConfig.comingSoon"
                          }
                        ].map((opt) => (
                          <button
                            key={opt.value}
                          ...
                          >
                            ...
                            <span className="text-muted-foreground text-[10px]">
                              {opt.comingSoon
                                ? t((opt as { disabledLabelKey?: string }).disabledLabelKey ?? "agentConfig.comingSoon")
                                : t(opt.descKey)}
                            </span>
                          </button>
                        ))}
                      ...
                      {shouldSuggestUnsetAnthropicApiKey && (
                        <div className="rounded-md border border-amber-300/60 bg-amber-50/40 px-2.5 py-2 space-y-2">
                          <p className="text-[11px] text-amber-900/90 leading-relaxed">
                            {t("onboarding.claudeFailedWithKey")}
                          </p>
                          <Button
                            size="sm"
                            variant="outline"
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={t("onboarding.taskDescPlaceholder")}
...
function AdapterEnvironmentResult({
  ...
  const statusLabel =
    result.status === "pass"
      ? t("agentConfig.statusPassed")
      : result.status === "warn"
      ? t("agentConfig.statusWarnings")
      : t("agentConfig.statusFailed");
  const statusClass =
    result.status === "pass"
