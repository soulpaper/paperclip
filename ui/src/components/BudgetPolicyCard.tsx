...
export function BudgetPolicyCard({
  ...
  const windowLabel = (windowKind: BudgetPolicySummary["windowKind"]) =>
    windowKind === "lifetime" ? t("costs.lifetimeBudget") : t("costs.monthlyUtcBudget");
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("costs.percentOfLimit", { percent: summary.utilizationPercent }) : t("costs.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("costs.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("costs.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("costs.softAlertAt", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${t("costs.pauseReasonSuffix", { reason: summary.pauseReason })}` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("costs.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("costs.pausedProjectMessage")
          : t("costs.pausedScopeMessage")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("costs.saving") : summary.amount > 0 ? t("costs.updateBudget") : t("costs.setBudget")}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("costs.invalidDollarAmount")}</p>
        ) : null}
      </div>
    );
  }
