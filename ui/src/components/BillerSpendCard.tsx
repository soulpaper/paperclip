        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> {t("billerSpend.tokensIn")}
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> {t("billerSpend.tokensOut")}
              {" · "}
              {t("billerSpend.providerCount", { count: row.providerCount })}
              {" · "}
              {t("billerSpend.modelCount", { count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {budgetMonthlyCents > 0 && (
          <QuotaBar
            ...
            rightLabel={t("billerSpend.ofAllocation", { percent: Math.round(budgetPct) })}
          />
        )}
        ...
        <div className="text-xs text-muted-foreground">
          ...
          {" · "}
          {formatCents(weekSpendCents)} {t("billerSpend.thisWeek")}
        </div>
                  ...
                  <div key={entry.provider} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <div className="text-muted-foreground">
                        {formatTokens(entry.inputTokens + entry.outputTokens)} {t("billerSpend.tok")}
                      </div>
                    </div>
                  </div>
                ))}
