...
const DAYS_OF_WEEK_VALUES = ["1", "2", "3", "4", "5", "6", "0"] as const;
...
function describeSchedule(cron: string, t?: (key: string, opts?: Record<string, string>) => string): string {
  ...
  if (!t) {
    // Fallback: English strings (used when called outside React)
    switch (preset) {
      case "every_minute":
        return "Every minute";
      case "every_hour":
      ...
      case "weekly": {
        const dayLabels: Record<string, string> = { "0": "Sun", "1": "Mon", "2": "Tue", "3": "Wed", "4": "Thu", "5": "Fri", "6": "Sat" };
        return `Every ${dayLabels[dayOfWeek] ?? dayOfWeek} at ${timeStr}`;
      }
      case "monthly":
  ...
  switch (preset) {
    ...
    case "every_hour":
      return t("scheduleEditor.everyHourAt", { minute: minute.padStart(2, "0") });
    case "every_day":
      return t("scheduleEditor.everyDayAt", { time: timeStr });
    case "weekdays":
      return t("scheduleEditor.weekdaysAt", { time: timeStr });
    case "weekly": {
      const dayKey = `scheduleEditor.day${dayOfWeek}` as const;
      return t("scheduleEditor.everyDayOfWeekAt", { day: t(dayKey), time: timeStr });
    }
    case "monthly":
      return t("scheduleEditor.monthlyAt", { day: dayOfMonth, time: timeStr });
    case "custom":
...
export function ScheduleEditor({
  ...
  const PRESETS: { value: SchedulePreset; label: string }[] = [
    { value: "every_minute", label: t("schedule.everyMinute") },
    { value: "every_hour", label: t("schedule.everyHour") },
    { value: "every_day", label: t("schedule.everyDay") },
    { value: "weekdays", label: t("schedule.weekdays") },
    { value: "weekly", label: t("schedule.weekly") },
    { value: "monthly", label: t("schedule.monthly") },
    { value: "custom", label: t("schedule.customCron") },
  ];

  const DAYS_OF_WEEK = DAYS_OF_WEEK_VALUES.map((v) => ({
    value: v,
    label: t(`scheduleEditor.day${v}`),
  }));
