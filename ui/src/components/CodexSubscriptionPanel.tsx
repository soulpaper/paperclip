...
function detailText(window: QuotaWindow, t: (key: string, opts?: Record<string, unknown>) => string): string | null {
  ...
  return t("codexSubscription.resetsAt", { date: formatted });
}
      ...
      <div className="flex items-start justify-between gap-3 border-b border-border pb-3">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">
            {t("codexSubscription.title")}
          </div>
          <div className="mt-1 text-sm text-muted-foreground">
            {t("codexSubscription.description")}
          </div>
        </div>
        {source ? (
      ...
      <div className="mt-4 space-y-5">
        <div className="space-y-3">
          <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
            {t("codexSubscription.accountWindows")}
          </div>
          <div className="space-y-3">
        ...
        {modelWindows.length > 0 ? (
          <div className="space-y-3">
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
              {t("codexSubscription.modelWindows")}
            </div>
            <div className="space-y-3">
    ...
    <div className="border border-border px-3.5 py-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 text-sm font-semibold tabular-nums text-foreground">
          {t("codexSubscription.usedPercent", { percent: window.usedPercent })}
        </div>
      </div>
