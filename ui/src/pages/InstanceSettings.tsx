  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.failedToDisableOne", { detail })
            : t("instanceSettings.failedToDisableMany", { count: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.failedToDisableAll"));
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instanceSettings.activeCount")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instanceSettings.disabledCount")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("instanceSettings.companySingular") : t("instanceSettings.companyPlural")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("instanceSettings.agentNoun") : t("instanceSettings.agentsNoun");
              if (!window.confirm(t("instanceSettings.confirmDisableAll", { count: enabledCount, noun }))) {
                return;
              }
