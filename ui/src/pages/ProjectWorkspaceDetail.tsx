...
function parseRuntimeConfigJson(value: string, errorMessages?: { mustBeObject: string; invalidJson: string }) {
  const trimmed = value.trim();
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: errorMessages?.mustBeObject ?? "Runtime services JSON must be a JSON object.",
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : (errorMessages?.invalidJson ?? "Invalid JSON."),
    };
  }
}
...
function validateWorkspaceForm(
  ...
  messages: {
    remoteManagedWorkspace: string;
    requiresPathOrUrl: string;
    absolutePath: string;
    repoUrl: string;
    mustBeObject: string;
    invalidJson: string;
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return messages.remoteManagedWorkspace;
    }
  } else if (!cwd && !repoUrl) {
    return messages.requiresPathOrUrl;
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return messages.absolutePath;
  }
  ...
  if (repoUrl) {
    try {
      ...
      return messages.repoUrl;
    }
  }

  const runtimeConfig = parseRuntimeConfigJson(form.runtimeConfig, { mustBeObject: messages.mustBeObject, invalidJson: messages.invalidJson });
  if (!runtimeConfig.ok) {
...
export function ProjectWorkspaceDetail() {
  ...
  const SOURCE_TYPE_OPTIONS: Array<{ value: ProjectWorkspaceSourceType; label: string; description: string }> = [
    { value: "local_path", label: t("projectWorkspaceDetail.sourceTypeLocalPath"), description: t("projectWorkspaceDetail.sourceTypeLocalPathDesc") },
    { value: "non_git_path", label: t("projectWorkspaceDetail.sourceTypeNonGitPath"), description: t("projectWorkspaceDetail.sourceTypeNonGitPathDesc") },
    { value: "git_repo", label: t("projectWorkspaceDetail.sourceTypeGitRepo"), description: t("projectWorkspaceDetail.sourceTypeGitRepoDesc") },
    { value: "remote_managed", label: t("projectWorkspaceDetail.sourceTypeRemoteManaged"), description: t("projectWorkspaceDetail.sourceTypeRemoteManagedDesc") },
  ];

  const VISIBILITY_OPTIONS: Array<{ value: ProjectWorkspaceVisibility; label: string }> = [
    { value: "default", label: t("projectWorkspaceDetail.visibilityDefault") },
    { value: "advanced", label: t("projectWorkspaceDetail.visibilityAdvanced") },
  ];
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("projectDetail.workspaces"), href: `/projects/${canonicalProjectRef}/workspaces` },
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("projectWorkspaceDetail.failedToSave"));
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("projectWorkspaceDetail.failedToUpdate"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, action) => {
      ...
      setRuntimeActionMessage(
        action === "stop"
          ? t("projectWorkspaceDetail.runtimeStopped")
          : action === "restart"
            ? t("projectWorkspaceDetail.runtimeRestarted")
            : t("projectWorkspaceDetail.runtimeStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : t("projectWorkspaceDetail.failedToControlRuntime"));
    },
  });

  if (projectQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("projectWorkspaceDetail.loadingWorkspace")}</p>;
  ...
  if (!project || !workspace || !form || !initialState) {
    return <p className="text-sm text-muted-foreground">{t("projectWorkspaceDetail.workspaceNotFound")}</p>;
  }

  const saveChanges = () => {
    const validationError = validateWorkspaceForm(form, {
      remoteManagedWorkspace: t("projectWorkspaceDetail.validationRemoteManagedWorkspace"),
      requiresPathOrUrl: t("projectWorkspaceDetail.validationRequiresPathOrUrl"),
      absolutePath: t("projectWorkspaceDetail.validationAbsolutePath"),
      repoUrl: t("projectWorkspaceDetail.validationRepoUrl"),
      mustBeObject: t("projectWorkspaceDetail.runtimeServicesJsonMustBeObject"),
      invalidJson: t("projectWorkspaceDetail.invalidJson"),
    });
    if (validationError) {
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-xs text-muted-foreground">
          {workspace.isPrimary ? t("projectWorkspaceDetail.primaryBadge") : t("projectWorkspaceDetail.secondaryBadge")}
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
              <div className="space-y-2">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                  {t("projectWorkspaceDetail.sectionHeader")}
                </div>
                <h1 className="text-2xl font-semibold">{workspace.name}</h1>
                <p className="max-w-2xl text-sm text-muted-foreground">
                  {t("projectWorkspaceDetail.description")}
                </p>
              </div>
              {!workspace.isPrimary ? (
                <Button
                ...
                <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
                  ...
                  {t("projectWorkspaceDetail.isPrimaryBadge")}
                </div>
              )}
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label={t("projectWorkspaceDetail.fieldWorkspaceName")}>
                <input
              ...
              <Field label={t("projectWorkspaceDetail.fieldVisibility")}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("projectWorkspaceDetail.fieldSourceType")} hint={sourceTypeDescription ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("projectWorkspaceDetail.fieldLocalPath")}>
                  <input
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspaceDetail.fieldRepoUrl")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
                ...
                <Field label={t("projectWorkspaceDetail.fieldRepoRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspaceDetail.fieldDefaultRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                ...
                <Field label={t("projectWorkspaceDetail.fieldSharedWorkspaceKey")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspaceDetail.fieldRemoteProvider")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                <Field label={t("projectWorkspaceDetail.fieldRemoteWorkspaceRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspaceDetail.fieldSetupCommand")} hint={t("projectWorkspaceDetail.setupCommandHint")}>
                  <textarea
                ...
                <Field label={t("projectWorkspaceDetail.fieldCleanupCommand")} hint={t("projectWorkspaceDetail.cleanupCommandHint")}>
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <Field label={t("projectWorkspaceDetail.fieldRuntimeServicesJson")} hint={t("projectWorkspaceDetail.runtimeServicesJsonHint")}>
                <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("projectWorkspaceDetail.noUnsavedChanges")}</p> : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("projectWorkspaceDetail.workspaceFacts")}</div>
              <h2 className="text-lg font-semibold">{t("projectWorkspaceDetail.currentState")}</h2>
            </div>
            <Separator className="my-4" />
            <DetailRow label={t("projectWorkspaceDetail.labelProject")}>
              <Link to={`/projects/${canonicalProjectRef}`} className="hover:underline">{project.name}</Link>
            </DetailRow>
            <DetailRow label={t("projectWorkspaceDetail.labelWorkspaceId")}>
              <span className="break-all font-mono text-xs">{workspace.id}</span>
            </DetailRow>
            <DetailRow label={t("projectWorkspaceDetail.labelLocalPath")}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("common.none")}</span>
            </DetailRow>
            <DetailRow label={t("projectWorkspaceDetail.labelRepo")}>
            ...
            <DetailRow label={t("projectWorkspaceDetail.labelDefaultRef")}>{workspace.defaultRef ?? t("common.none")}</DetailRow>
            <DetailRow label={t("projectWorkspaceDetail.labelUpdated")}>{new Date(workspace.updatedAt).toLocaleString()}</DetailRow>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("projectWorkspaceDetail.runtimeServicesHeader")}</div>
                <h2 className="text-lg font-semibold">{t("projectWorkspaceDetail.attachedServices")}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("projectWorkspaceDetail.attachedServicesDescription")}
                </p>
              </div>
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
                ...
                >
                  ...
                  {t("projectWorkspaceDetail.startButton")}
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  {t("projectWorkspaceDetail.restartButton")}
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  {t("projectWorkspaceDetail.stopButton")}
                </Button>
              </div>
            </div>
            <Separator className="my-4" />
            {workspace.runtimeServices && workspace.runtimeServices.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="space-y-1 text-xs text-muted-foreground">
                          ...
                          {service.port ? <div>{t("projectWorkspaceDetail.port", { port: service.port })}</div> : null}
                          <div>{service.command ?? t("projectWorkspaceDetail.noCommandRecorded")}</div>
              ...
              <p className="text-sm text-muted-foreground">
                {workspace.runtimeConfig?.workspaceRuntime
                  ? t("projectWorkspaceDetail.noRuntimeServicesRunning")
                  : t("projectWorkspaceDetail.noRuntimeConfigured")}
              </p>
            )}
          </div>
        </div>
