...
function FrontmatterCard({ data }: { data: FrontmatterData }) {
  const fieldLabels = useFrontmatterFieldLabels();
  return (
...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyImport.selectFileToPreview")} />
    );
  }
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("companyImport.renameItems", { count: conflicts.length, plural: conflicts.length === 1 ? "" : "s" })}
          </span>
        </div>
    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {agents.length} {agents.length === 1 ? t("companies.agent") : t("companies.agents")}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
export function CompanyImport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("companyImport.importSource") },
    ]);
  }, [setBreadcrumbs]);
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github" as const, icon: Github, label: t("companyImport.githubRepo") },
              { key: "local" as const, icon: Upload, label: t("companyImport.localZip") },
            ]
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("companyImport.withFiles", {
                    name: localPackage.name,
                    count: Object.keys(localPackage.files).length,
                    plural: Object.keys(localPackage.files).length === 1 ? "" : "s",
                  })}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("companyImport.conflicts", { count: conflicts.length, plural: conflicts.length === 1 ? "" : "s" })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("companyImport.errors", { count: importPreview.errors.length, plural: importPreview.errors.length === 1 ? "" : "s" })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("companyImport.importFiles", { count: selectedCount, plural: selectedCount === 1 ? "" : "s" })}
            </Button>
          </div>
