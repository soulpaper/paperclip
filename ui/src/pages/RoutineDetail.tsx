  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("routineDetail.nextRun", { time: new Date(trigger.nextRunAt).toLocaleString() })
            : trigger.kind === "webhook"
              ? t("routineDetail.webhookLabel")
              : t("routineDetail.apiLabel")}
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("routineDetail.lastResult")} {trigger.lastResult}</span>}
...
export function RoutineDetail() {
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.failedToSaveBody"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routineDetail.routineRunStarted"), tone: "success" });
      setActiveTab("runs");
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.routineRunFailed"),
        body: error instanceof Error ? error.message : t("routineDetail.routineRunFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.failedToUpdate"),
        body: error instanceof Error ? error.message : t("routineDetail.failedToUpdateBody"),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.failedToAddTriggerBody"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.failedToUpdateTriggerBody"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.failedToDeleteTriggerBody"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.failedToRotateSecretBody"),
        tone: "error",
      });
    },
  });
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : t("routineDetail.routineNotFound")}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived" ? t("routineDetail.statusArchived") : automationEnabled ? t("routineDetail.statusActive") : t("routineDetail.statusPaused");
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <textarea
          ...
          placeholder={t("routineDetail.routineTitlePlaceholder")}
          rows={1}
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routineDetail.webhookUrl"), secretMessage.webhookUrl)}>
                ...
                {t("routineDetail.urlButtonLabel")}
              </Button>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routineDetail.webhookSecret"), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("routineDetail.secretButtonLabel")}
              </Button>
            </div>
          </div>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3">
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {kind}{kind === "webhook" ? ` — ${t("routineDetail.comingSoon")}` : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routineDetail.noActivityYet")}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
