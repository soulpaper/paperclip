  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        {project.targetDate && (
          <div>
            <span className="text-muted-foreground">{t("common.targetDate")}</span>
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(0,18rem)_minmax(0,1fr)_auto] md:items-start">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
              ...
              <span className="rounded-full border border-border px-2 py-0.5 text-[11px]">
                {t("projectDetail.servicesRunning", { running: summary.runningServiceCount, total: summary.serviceCount })}
              </span>
          ...
          <div className="min-w-0">
            <div className="mb-2 text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {t("projectDetail.issuesCount", { count: summary.issues.length })}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                <span className="inline-flex items-center rounded-md border border-dashed border-border px-2.5 py-1.5 text-xs text-muted-foreground">
                  {t("projectDetail.andMore", { count: hiddenIssueCount })}
                </span>
              ) : null}
            </div>
          </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      ...
      if (archived) {
        pushToast({ title: t("projectDetail.hasBeenArchived", { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("projectDetail.hasBeenUnarchived", { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
