...
export function InstanceGeneralSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t('instanceSettings.failedToUpdateGeneral'));
    },
  });

  if (generalQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t('instanceSettings.loadingGeneral')}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {generalQuery.error instanceof Error
          ...
          : t('instanceSettings.failedToLoadGeneral')}
      </div>
    );
  }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t('instanceSettings.generalDesc')}
        </p>
      </div>
